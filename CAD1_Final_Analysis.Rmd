...
rm(list=ls(all=TRUE))
setwd('C:/Users/musscb/OneDrive - University of Leeds/Cadenza Project/Listening Panel/data')
library(tidyverse)  # Efficient package for tidy data
library(lmerTest)   # Leading package for mixed effects models
library(ggpubr)     # For multiplot visualisations
library(ggcorrplot) # Easy correlation matrix visualisation
library(ggdist)     # For further specialised visualisations
library(scales)     # For rescaling variables
library(rstatix)    # For RM ANOVA
library(emmeans)    # For post-hoc contrasts in linear mixed effects models
library(glmmTMB)    # For zero-one inflated beta regression
library(marginaleffects)
library(modelsummary)  # For summarising various model indices
library(DHARMa)        # Simulate model residuals for diagnostics (LMM/GLMM)
library(multcomp)      # Post-hoc package
library(performance)   # For checking model performance and assumptions
library(ggeffects)
...
Boxplot of BAQ vs. HAAQI across systems:

```{r haaqi, echo=FALSE, warning=FALSE}
...
Calculate Spearman rank correlations between BAQ and HAAQI:

```{r spearman, echo=FALSE, warning=FALSE}
...
knitr::kable(spearman, row.names = FALSE)
```

### Basic Descriptives

Attributes means by system:
...
```{r meanHL, echo=FALSE, warning=FALSE}
knitr::kable(df %>%
  group_by(Severity) %>%
  summarise(BAQ = round(mean(audio_quality_overall, na.rm=T), digits = 2), 
            Clarity = round(mean(clarity, na.rm = T), digits = 2), 
...
```{r attr, echo=FALSE, warning=FALSE}
viz <- df %>%
...
```{r glmm, echo=TRUE, warning=FALSE}
...
```{r summ, echo=FALSE, warning=FALSE}
modelsummary(list(glmm, lmm)) 
```

Compare performance metrics between models - using "performance" package

```{r comp, echo=FALSE, warning=FALSE}
compare_performance(glmm, lmm, rank = TRUE, verbose = FALSE)
```

Check residuals, outliers, dispersion issues (1st for GLMM, 2nd for alt LMM):

```{r dharma, echo=FALSE, warning=FALSE}
...
```{r glmm_summ, echo=FALSE, warning=FALSE}
summary(glmm) # Check model characteristics, parameters, effects structures
```

### **Post-hoc comparisons**

Using the "emmeans" package, first look at system effects:

```{r em_system, echo=FALSE, warning=FALSE}
...
```{r em_HL, echo=FALSE, warning=FALSE}
...
Finally, system and HL interactions:

```{r em_int, echo=FALSE, warning=FALSE}
...
```{r baq_viz, echo=FALSE, warning=FALSE}
df %>%
  group_by(Team) %>%
...
```{r baq_hl, echo=FALSE, warning=FALSE}
df %>%
  group_by(Severity) %>%
  summarise_at(vars(audio_quality_overall), funs(mean(., na.rm=TRUE)))
...
Visualise interactions between system and HL severity:

```{r baq_int, echo=FALSE, warning=FALSE}
...
```{r pca_prep, echo=TRUE, warning=FALSE}
...
```{r assum_1, echo=TRUE, warning=FALSE}
...
Clean data to remove problematic variables, and then check assumptions again:

```{r assum_2, echo=TRUE, warning=FALSE}
...
Clean data again to remove "crest factor", and final assumption check:

```{r assum_3, echo=TRUE, warning=FALSE}
...
```{r eig, echo=FALSE, warning=FALSE}
pcdat$eig   
```

Scree plot (suggesting 2 components):

```{r scree, echo=FALSE, warning=FALSE}
fviz_eig(pcdat, addlabels = TRUE, ylim = c( 0, 60)) 
```

Parallel analysis (suggesting 2 components):

```{r para, echo=FALSE, warning=FALSE}
paran(dfNorm)
```

Check communalities (easiest to replicate using "principal" function)

```{r comm, echo=FALSE, warning=FALSE}
...
Visualise variance and then contribution:

```{r varcon, echo=FALSE, warning=FALSE}
...
```{r con_viz, echo=FALSE, warning=FALSE}
...
```{r supp_pca, echo=FALSE, warning=FALSE}
...
### **Run final PCA with varimax rotation for improved interpretation**

```{r finalpca, echo=TRUE, warning=FALSE}
pcmax <- psych::principal(dfNorm, nfactors = 2, rotate = 'varimax')
pcmax$fit
pcmax$loadings
```

We then add component scores to our main dataset, for visualising and modelling.

```{r pca_add, echo=FALSE, warning=FALSE}
df2 <- df2 %>%
...
### **Characterise ML systems based on PCA components**

```{r pca_sys, echo=FALSE, warning=FALSE}
...
```{r pca_zoom, echo=FALSE, warning=FALSE}
zoom <- df2
...
### **Run GLMM with PCs as predictors of BAQ**

```{r models, echo=TRUE, warning=FALSE}
